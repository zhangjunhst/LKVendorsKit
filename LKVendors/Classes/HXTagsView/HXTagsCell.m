...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_tagAttribute.tagMinWidth) {
        return CGSizeMake(MAX(frame.size.width, _tagAttribute.tagMinWidth) + _tagAttribute.tagSpace, layout.itemSize.height);
    }
...
+ (CGFloat)getCellHeightWithTags:(NSArray *)tags layout:(HXTagCollectionViewFlowLayout *)layout tagAttribute:(HXTagAttribute *)tagAttribute width:(CGFloat)width
{
    ...
    for (NSInteger i = 0; i < itemCount; i++) {
        ...
        if (tagAttribute.tagMinWidth) {
            itemSize = CGSizeMake(MAX(frame.size.width, tagAttribute.tagMinWidth) + tagAttribute.tagSpace, layout.itemSize.height);
        }
