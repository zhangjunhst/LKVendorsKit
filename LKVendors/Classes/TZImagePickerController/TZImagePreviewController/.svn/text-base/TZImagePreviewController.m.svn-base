...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[TZVideoPreviewCell class] forCellWithReuseIdentifier:@"TZVideoPreviewCellForNet"];// FIX:解决数据源中混有phset和NSURL时视频复用问题
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([photo isKindOfClass:[PHAsset class]]) {
        ...
        if ([photo isKindOfClass:[UIImage class]]) {
            ...
            NSString *fileTyp = [URL.fileType lowercaseString];
            NSString *suffix = [[URL.absoluteString.lowercaseString componentsSeparatedByString:@"."] lastObject];
            BOOL noramal_video_url = suffix && [self.videoSuffixs containsObject:suffix];
            BOOL lk_video_url = fileTyp && [self.videoSuffixs containsObject:fileTyp];
            
            if (noramal_video_url || lk_video_url) {
                
                cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"TZVideoPreviewCellForNet" forIndexPath:indexPath];
