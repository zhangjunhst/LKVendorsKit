...
#define FS_UIColorFromRGB(rgbValue) [UIColor colorWithRed:((float)((rgbValue & 0xFF0000) >> 16))/255.0 green:((float)((rgbValue & 0xFF00) >> 8))/255.0 blue:((float)(rgbValue & 0xFF))/255.0 alpha:1.0]
...
static const NSCalendarUnit unitFlagsAll = (NSCalendarUnitYear | NSCalendarUnitQuarter | NSCalendarUnitMonth | NSCalendarUnitWeekOfYear | NSCalendarUnitWeekOfMonth | NSCalendarUnitDay | NSCalendarUnitHour | NSCalendarUnitMinute | NSCalendarUnitSecond | NSCalendarUnitEra | NSCalendarUnitWeekday | NSCalendarUnitWeekdayOrdinal | NSCalendarUnitWeekOfYear);
...
@property (weak  , nonatomic) UILabel                    *monthHolder;
...
- (void)initialize
{
    ...
    UILabel *monthHolder = [UILabel new];
    monthHolder.textColor = [FS_UIColorFromRGB(0x4880FF) colorWithAlphaComponent:0.05];
    monthHolder.font = [UIFont boldSystemFontOfSize: (137)];
    monthHolder.backgroundColor = [[UIColor whiteColor] colorWithAlphaComponent:0];
    monthHolder.text = @([self getMonth:_currentPage]).description;
    monthHolder.textAlignment = NSTextAlignmentCenter;
    [daysContainer addSubview:monthHolder];
    self.monthHolder = monthHolder;
        ...
        if (!self.floatingMode) {
            switch (self.transitionCoordinator.representingScope) {
                case FSCalendarScopeMonth: {
                    ...
                    _monthHolder.frame = CGRectMake(0, 0, self.fs_width, contentHeight);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (self.floatingMode && _collectionView.indexPathsForVisibleItems.count) {
        ...
        if (![self.gregorian isDate:currentPage equalToDate:_currentPage toUnitGranularity:NSCalendarUnitMonth]) {
            ...
            [self configureAppearanceForMonthHolder];
            [self configureAppearanceForWeekdayLabels];
        }
...
- (NSInteger)getWeekOfYear:(NSDate *)date{
    unsigned int unitFlags = unitFlagsAll;
    NSDateComponents *dateComponents = [self.gregorian components:unitFlags fromDate:date];
    return [dateComponents weekOfYear];
}

- (void)configureAppearanceForWeekdayLabels{
    // 先全部重置
    [self.calendarWeekdayView.weekdayLabels enumerateObjectsUsingBlock:^(UILabel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        obj.textColor = self.appearance.weekdayTextColor;
    }];
    // 只针对当前页的选中日期就行头部更新
    [self.selectedDates enumerateObjectsUsingBlock:^(NSDate * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        
        BOOL shouldChange = NO;
        if (self.scope == FSCalendarScopeMonth) {
            shouldChange = [self getMonth:obj] == [self getMonth:self.currentPage];
        }else if (self.scope == FSCalendarScopeWeek){
            shouldChange = [self getWeekOfYear:obj] == [self getWeekOfYear:self.currentPage];
        }
        
        if (shouldChange) {
            NSUInteger index;
            NSInteger delt = [self getWeekday:obj] - self.firstWeekday;
            if ([self getWeekday:obj] > self.firstWeekday) {
                index = delt;
            }else if (delt == 0) {
                index = 0;
            }else{
            ...
            [self.calendarWeekdayView.weekdayLabels enumerateObjectsUsingBlock:^(UILabel * _Nonnull obj2, NSUInteger idx2, BOOL * _Nonnull stop2) {
                obj2.textColor = idx2 == index ? self.appearance.eventSelectionColor : obj2.textColor;
            }];
        }
    }];
}

- (void)configureAppearanceForMonthHolder{
    
    if (_monthHolder) {
        NSInteger fs_month = [self getMonth:_currentPage];
        _monthHolder.text = @(fs_month).description;
    }
}
