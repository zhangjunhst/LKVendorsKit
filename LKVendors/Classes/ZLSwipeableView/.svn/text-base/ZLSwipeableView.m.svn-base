...
- (void)removeTopViewToCrashAnimated:(BOOL)animated{
    
    if (_scheduler.timer.isValid) {
        NSLog(@"⚠️操作太频繁!!!");
        return;
    }
    ...
    [UIView animateWithDuration:animated ? 0.5 : 0 animations:^{
        CGAffineTransform transForm = CGAffineTransformMakeTranslation(-[UIScreen mainScreen].bounds.size.width, 0);
        topView.transform = transForm;
        
    } completion:^(BOOL finished) {
        self.userInteractionEnabled = YES;
        [self remove:topView];
        if ([self.mutableHistory containsObject:topView]) {
            NSLog(@"历史数组中存在");
            [self.mutableHistory removeObject:topView];
        }
        [topView removeFromSuperview];
        
        
        if (_delegate &&
            [_delegate respondsToSelector:@selector(swipeableView:didEndRemoveView:)]) {
            [_delegate swipeableView:self didEndRemoveView:topView];
        }
...
- (void)rewind {
    ...
    if (_delegate &&
        [_delegate respondsToSelector:@selector(swipeableView:didEndRewindView:)]) {
        [_delegate swipeableView:self didEndRewindView:viewToBeRewinded];
    }
...
- (void)updateViews {
    ...
    for (NSUInteger i = 0; i < activeViews.count; i++) {
        ...
        if (shouldBeHidden) {
            if (_delegate &&
                [_delegate respondsToSelector:@selector(swipeableViewDidEndUpdateViews:)]) {
                [_delegate swipeableViewDidEndUpdateViews:self];
            }
            continue;
        }
...
- (void)swipeView:(UIView *)view
  ...
  directionVector:(CGVector)directionVector {
    ...
    if (_delegate &&
        [_delegate respondsToSelector:@selector(swipeableView:didEndSwipeView:direction:)]) {
        
        if(direction == ZLSwipeableViewDirectionDown){
            [_delegate swipeableView:self didEndSwipeView:view direction:UISwipeGestureRecognizerDirectionDown];
        }
    }
