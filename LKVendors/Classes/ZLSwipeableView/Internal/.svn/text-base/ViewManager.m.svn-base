...
- (instancetype)initWithView:(UIView *)view
    ...
    if (self) {
        ...
        [view addGestureRecognizer:[[SwipeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipe:) touches:1 direction:(UISwipeGestureRecognizerDirectionUp)]];
        [view addGestureRecognizer:[[SwipeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipe:) touches:1 direction:(UISwipeGestureRecognizerDirectionDown)]];
        [view addGestureRecognizer:[[SwipeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipe:) touches:1 direction:(UISwipeGestureRecognizerDirectionRight)]];
        [view addGestureRecognizer:[[SwipeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipe:) touches:1 direction:(UISwipeGestureRecognizerDirectionLeft)]];
...
- (void)dealloc {
    for (UIGestureRecognizer *aGestureRecognizer in _view.gestureRecognizers) {
        if ([aGestureRecognizer isKindOfClass:[PanGestureRecognizer class]] || [aGestureRecognizer isKindOfClass:[SwipeGestureRecognizer class]]) {
...
- (void)handleSwipe:(UISwipeGestureRecognizer *)recognizer{
    ...
    if ([self.swipeableView.delegate
         respondsToSelector:@selector(swipeableView:canSwipeView:direction:)]) {

        BOOL canSwipe = [self.swipeableView.delegate swipeableView:self.swipeableView
                                                      canSwipeView:_view
                                                         direction:recognizer.direction];
        if (canSwipe) {
            if ([self.swipeableView.delegate
                 respondsToSelector:@selector(swipeableView:didSwipeView:direction:)]) {
                [self.swipeableView.delegate swipeableView:self.swipeableView
                                              didSwipeView:_view
                                                 direction:recognizer.direction];
            }
        }else{
            NSLog(@"swipeableView:canSwipeView:direction: 返回为NO");
        }
    }else{
        NSLog(@"请实现swipeableView:canSwipeView:direction:代理方法");
    }
}
