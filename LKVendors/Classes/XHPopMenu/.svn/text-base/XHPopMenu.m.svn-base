...
- (UIImageView *)menuContainerView {
    if (!_menuContainerView) {
        ...
        UIImage *bgImage = [[UIImage imageNamed:@"MoreFunctionFrameBlue"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        _menuContainerView = [[UIImageView alloc] initWithImage:[bgImage resizableImageWithCapInsets:UIEdgeInsetsMake(30, 10, 60, 50) resizingMode:UIImageResizingModeTile]];
        _menuContainerView.userInteractionEnabled = YES;
        [_menuContainerView setTintColor:[UIColor whiteColor]];
        CGFloat width = MAX(self.maxWidth, kXHMenuTableViewWidth);
        
        CGFloat naviH = 44 + ([self isAlienScreen] ? 44 : 20);
        
        _menuContainerView.frame = CGRectMake(CGRectGetWidth(self.bounds) - width - 6, naviH, width, self.menus.count * (kXHMenuItemViewHeight + kXHSeparatorLineImageViewHeight) + kXHMenuTableViewSapcing);
        
        _menuContainerView.layer.shadowColor = [UIColor colorWithRed:0/255.0 green:0/255.0 blue:0/255.0 alpha:0.2].CGColor;
        _menuContainerView.layer.shadowOffset = CGSizeMake(0,2);
        _menuContainerView.layer.shadowOpacity = 1;
        _menuContainerView.layer.shadowRadius = 10;
...
- (void)setup {
    ...
    __block CGFloat maxWidth = kXHMenuTableViewWidth;
    
    [self.menus enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        XHPopMenuItem *objItem = (XHPopMenuItem *)obj;
        
        CGFloat width = [self widthOfString:objItem.title withFont:[UIFont systemFontOfSize:14]] + kXHMenuItemViewHeight + kXHMenuItemViewImageSapcing * 2;
        maxWidth = MAX(maxWidth, width);
    }];
    
    [self.menus enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        XHPopMenuItem *objItem = (XHPopMenuItem *)obj;
        objItem.maxWidth = maxWidth;
    }];
